				...
				<div className='flex flex-wrap items-center justify-between gap-3 p-4 border-b border-gray-200 dark:border-gray-800'>
					<div className='flex flex-wrap gap-3'>
						<input
							...
							placeholder={t('borrowing.search')}
					...
					<Button onClick={openIssue} size='sm'>
						+ {t('borrowing.issue')}
					</Button>
				</div>
								...
								borrows.map((b) => (
									...
									>
										...
										<TableCell className='px-4 py-3'>
											{statusBadge(b.status, t(`status.${b.status}`))}
										</TableCell>
										<TableCell className='px-4 py-3'>
											<div className='flex gap-2'>
												{b.status === 'borrowed' && (
													<button
														onClick={() => overdue(b.id)}
			...
			>
						...
						>
							...
							{students.map((s: any) => (
								<option key={s.id} value={s.id}>
									{s.student_id} —{' '}
									{i18n.language === 'km' ? s.name_kh : s.name_en}
								</option>
							))}
						</select>
					</div>
						...
						>
							...
							{items.map((i: any) => (
								<option key={i.id} value={i.id}>
									{i.name} ({i.available_quantity} {t('table.available')})
								</option>
							))}
						</select>
					</div>
				...
				<div className='mt-6 flex justify-end gap-3'>
					...
					<Button
						onClick={issue}
						disabled={
							saving || !form.student_id || !form.staff_id || !form.item_id
						}
					>
