    ...
    <>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        <div className="flex justify-end p-4 border-b border-gray-200 dark:border-gray-800">
          <Button onClick={openAdd} size="sm">+ {t("items.add")}</Button>
        </div>
        <div className="overflow-x-auto">
          <Table>
            <TableHeader className="bg-gray-50 dark:bg-gray-800/50">
              <TableRow>
                {[t("table.no"), t("items.name"), t("table.quantity"), t("table.available"), t("table.actions")].map(h => (
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
              {loading ? (
                ...
                <TableRow key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                  <TableCell className="px-4 py-3 text-gray-500">{i + 1}</TableCell>
                  <TableCell className="px-4 py-3 font-medium text-gray-800 dark:text-white">{item.name}</TableCell>
                  <TableCell className="px-4 py-3 text-gray-600 dark:text-gray-400">{item.quantity}</TableCell>
                  <TableCell className="px-4 py-3">
                    <Badge color={item.available_quantity === 0 ? "error" : "success"} size="sm">
                  ...
                  <TableCell className="px-4 py-3">
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => openDel(item)}  className="text-xs text-error-500 hover:underline">{t("actions.delete")}</button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Modal isOpen={modal === "add" || modal === "edit"} onClose={close} className="max-w-sm p-6 m-4">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input id="qty" type="number" value={form.quantity} onChange={e => setForm(p => ({ ...p, quantity: Number(e.target.value), available_quantity: Number(e.target.value) }))} />
          </div>
          {modal === "edit" && (
            <div>
              <Label htmlFor="avail">{t("items.available")}</Label>
              <Input id="avail" type="number" value={form.available_quantity} onChange={e => setForm(p => ({ ...p, available_quantity: Number(e.target.value) }))} />
            </div>
          )}
        </div>
      ...
      <Modal isOpen={modal === "delete"} onClose={close} className="max-w-sm p-6 m-4">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{t("items.delete")}</h2>
