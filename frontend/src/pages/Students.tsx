...
export default function Students() {
	...
	const borrowBadge = (status?: string) => {
		...
		const map: Record<string, 'info' | 'error'> = {
			borrowed: 'info',
			overdue: 'error',
		};
		return (
									...
									>
										...
										<TableCell className='px-4 py-3 font-medium text-gray-800 dark:text-white'>
											...
											<div className='text-xs text-gray-400'>
												{i18n.language === 'km' ? s.name_en : s.name_kh}
											</div>
										</TableCell>
										<TableCell className='px-4 py-3 text-gray-600 dark:text-gray-400'>
											{s.group}
										...
										<TableCell className='px-4 py-3'>
											<Badge
												color={s.is_active ? 'success' : 'light'}
												size='sm'
											>
												{s.is_active
													? t('status.active')
				...
				<div className='grid grid-cols-2 gap-4'>
					{(
						...
						<div
							key={key}
							className={
								key === 'name_kh' || key === 'email' ? 'col-span-2' : ''
							}
						>
