...
const statusBadge = (status: string, label: string) => {
	...
	return (
		<Badge color={map[status] ?? 'primary'} size='sm'>
			{label}
		</Badge>
	);
};
...
export default function Dashboard() {
	...
	return (
		<>
			<PageMeta
				title='Dashboard — Setec'
				description='Setec uniform lending dashboard'
			/>
			...
			{loading ? (
				...
				<div className='space-y-6'>
					...
					<div className='grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6'>
						...
						/>
						<StatCard
							label={t('dashboard.totalReturned')}
						...
						/>
						<StatCard
							label={t('dashboard.totalAvailable')}
