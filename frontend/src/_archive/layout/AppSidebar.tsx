...
const AppSidebar: React.FC = () => {
	...
	const toggleSubmenu = (idx: number, mt: 'main' | 'others') =>
		setOpenSubmenu((p) =>
			p?.type === mt && p?.index === idx ? null : { type: mt, index: idx },
		);
			...
			{items.map((nav, idx) => (
				<li key={nav.name}>
					{nav.subItems ? (
						...
						>
							<span
								className={`menu-item-icon-size ${openSubmenu?.type === mt && openSubmenu?.index === idx ? 'menu-item-icon-active' : 'menu-item-icon-inactive'}`}
							>
								{nav.icon}
							</span>
							...
							{(isExpanded || isHovered || isMobileOpen) && (
								<ChevronDownIcon
									className={`ml-auto w-5 h-5 transition-transform duration-200 ${openSubmenu?.type === mt && openSubmenu?.index === idx ? 'rotate-180 text-brand-500' : ''}`}
								/>
							)}
						</button>
					) : (
					...
					{nav.subItems && (isExpanded || isHovered || isMobileOpen) && (
						<div
							ref={(el) => {
								subMenuRefs.current[`${mt}-${idx}`] = el;
							}}
							className='overflow-hidden transition-all duration-300'
							style={{
								height:
									openSubmenu?.type === mt && openSubmenu?.index === idx
										? `${subMenuHeight[`${mt}-${idx}`]}px`
										: '0px',
							}}
						>
								...
								{nav.subItems.map((sub) => (
									<li key={sub.name}>
										...
										>
											...
											{(sub.new || sub.pro) && (
												<span
													className={`ml-auto ${isActive(sub.path) ? 'menu-dropdown-badge-active' : 'menu-dropdown-badge-inactive'} menu-dropdown-badge`}
												>
													{sub.new ? 'new' : 'pro'}
												</span>
											)}
	...
	return (
		...
		>
			...
			<div className='flex flex-col overflow-y-auto duration-300 ease-linear no-scrollbar'>
								...
								{(isExpanded || isHovered || isMobileOpen) && user && (
									<li className='mt-4 pt-4 border-t border-gray-200 dark:border-gray-800'>
										<button
											onClick={() => {
												logout();
												navigate('/setec/login');
											}}
											className='menu-item group menu-item-inactive w-full text-red-500 hover:text-red-600'
										>
				...
				{(isExpanded || isHovered || isMobileOpen) && !isSetec ? (
					<SidebarWidget />
				) : null}
			</div>
		</aside>
